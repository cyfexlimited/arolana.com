...
from .admin_email import VendorEmailAdminMixin
from .models import (
    VendorBankAccount,
...
class VendorProfileAdmin(VendorEmailAdminMixin, admin.ModelAdmin):
    list_display = [
        'store_name',
        'user',
        'vendor_type',
    ...
    fieldsets = (
        ...
        ('Controlled Direct Contact', {
            ...
            'description': (
                'Backend-gated customer contact. Phone/WhatsApp are exposed only when vendor opts in, '
                'the subscription plan allows it, and the vendor is approved, verified, and active.'
            )
        }),
        ('Performance Metrics', {
            'fields': (
    ...
    def banner_preview(self, obj):
        if obj.store_banner:
            return format_html(
                '<img src="{}" width="160" height="52" style="border-radius: 12px; object-fit: cover;" />',
    ...
    def marketplace_location(self, obj):
        if getattr(obj, 'address_is_complete', False):
            return format_html(
                ...
                obj.location_label or obj.country,
            )
    ...
    def mark_unverified(self, request, queryset):
        ...
        self.message_user(request, f'{updated} seller profile(s) marked unverified.')
    ...
    def approve_vendors(self, request, queryset):
        ...
        self.message_user(request, f'{updated} vendor profile(s) approved.')
    ...
    def reject_vendors(self, request, queryset):
        ...
        self.message_user(request, f'{updated} vendor profile(s) rejected.')
    ...
    def suspend_vendors(self, request, queryset):
        ...
        self.message_user(request, f'{updated} vendor profile(s) suspended.')

    suspend_vendors.short_description = 'Suspend selected vendors'

    def activate_vendors(self, request, queryset):
        updated = queryset.update(is_active=True)
        self.message_user(request, f'{updated} vendor profile(s) activated.')
...
class VendorReviewAdmin(admin.ModelAdmin):
    ...
    def approve_reviews(self, request, queryset):
        ...
        self.message_user(request, f'{updated} vendor review(s) approved.')
    ...
    def hide_reviews(self, request, queryset):
        ...
        for review in queryset.select_related('vendor'):
            review.vendor.update_rating()
        self.message_user(request, f'{updated} vendor review(s) hidden.')
...
class VendorCallbackRequestAdmin(admin.ModelAdmin):
    ...
    def assign_to_me(self, request, queryset):
        ...
        self.message_user(request, f'{updated} callback request(s) assigned to you.')
    ...
    def mark_contacted(self, request, queryset):
        ...
        self.message_user(request, f'{updated} callback request(s) marked contacted.')
    ...
    def mark_resolved(self, request, queryset):
        ...
        self.message_user(request, f'{updated} callback request(s) marked resolved.')

    mark_resolved.short_description = 'Mark selected callback requests resolved'

    def mark_closed(self, request, queryset):
        updated = queryset.update(status='closed')
        self.message_user(request, f'{updated} callback request(s) closed.')
...
class VendorBankAccountAdmin(admin.ModelAdmin):
    ...
    def verify_accounts(self, request, queryset):
        ...
        self.message_user(request, f'{updated} bank account(s) verified.')
