...
class MobileCustomerWebAccountAuthenticationTests(TestCase):
    ...
    @patch(
        "accounts.utils.otp_utils.send_otp_email",
        return_value=True,
    )
    def test_web_account_login_requires_otp_and_links_mobile_profile(
        self,
        _send_email,
    ):
        ...
        self.assertFalse(
            verified.json()["otp_required"]
        )

        # The raw token should be returned once to the client.
        self.assertTrue(
            verified.json()["api_token"]
        )

        customer = MobileCustomer.objects.get(
            user=self.user
        )
    ...
    def test_invalid_web_password_does_not_create_mobile_profile(
        self,
    ):
    ...
    def test_profile_endpoint_accepts_bearer_mobile_token(
        self,
    ):
        ...
        self.assertEqual(
            response.json()["customer"]["user_id"],
            self.user.id,
        )

    def test_customer_language_setting_persists_and_is_returned(
        self,
    ):
        customer = MobileCustomer.objects.create(
            user=self.user,
    ...
    def test_native_registration_creates_real_user_then_verifies_otp(
        self,
        _send_email,
    ):
        registration = self.post_json(
            ...
            {
                "full_name": (
                    "Native Arolana Customer"
                ),
                "email": (
                    "native.arolana.customer@gmail.com"
                ),
                "phone_number": (
                    "+2348098765432"
                ),
        ...
        self.assertTrue(
            registration.json()["otp_required"]
        )
        ...
        # Registration returns the raw token once
        # so the mobile client can store it.
        self.assertTrue(
            verified.json()["api_token"]
        )
