...
class KYCRecord(BaseModel):
    ...
    manufacturer_certificate_number = models.CharField(
        max_length=140,
        blank=True,
    )

    import_export_license_number = models.CharField(
        max_length=140,
        blank=True,
    )
    ...
    authorized_person_phone = models.CharField(
        max_length=50,
    )
    ...
    # Compliance
    pep_check = models.BooleanField(
        default=False,
        help_text=(
            "Politically Exposed Person check passed"
        ),
    )
    ...
    def __str__(self):
        return (
            f"KYC for {self.vendor.store_name} - "
            f"{self.kyc_status}"
        )
